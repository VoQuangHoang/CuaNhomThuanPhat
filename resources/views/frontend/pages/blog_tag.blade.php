...
<main class="site-main product-listing">
    <section class="page-heading" style="background-color: #f9f3f2;">
        <div class="container">
            ...
            <h1 class="page-title title-cate">Tag : {{$tag->name}}</h1>
        </div>
    </section>
    ...
    <section class="product-listing-container">
        <div class="container">
            <div class="row gx-5">
                    ...
                    <div class="product-filter">
                        ...
                        <div class="form-check">
                            ...
                            <label class="form-check-label">
                                ...
                                <input class="form-check-input" name="category" type="checkbox" value="{{$item->id}}" {{\Request('slug') == $item->slug && \Request::segment(1) == 'danh-muc-tin' ? 'checked' : '' }}>
                                <a href="{{route('home.blog_cate', $item->slug)}}">{{$item->name}}</a>
                            </label>
                            @endforeach
                        </div>
                ...
                <div class="col-lg-9">
                    @if (count($blogByTag) > 0)
                    <div class="news-listing">
                        @foreach ($blogByTag as $item)
                        <div class="news-item">
                            <div class="thumbnail">
                    ...
                    <nav class="page-pagination">
                        {{ $blogByTag->links('vendor.pagination.bootstrap-5') }}
                    </nav>

                    @else
                    <span>Chưa có bài viết phù hợp</span>
                    @endif
                </div>
            </div>
        </div>
